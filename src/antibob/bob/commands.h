...
CONSOLE_COMMAND("kick_events", "s[comma sep event ids]", ComKickEvents, this, "kick all players once that triggered the given events or more")
CONSOLE_COMMAND("auto_punish_event", "s[event name]s['off'|'kick'|'ban']", ComAutoPunishEvent, this, "configure auto punish as soon as the event gets detected")
...
CONSOLE_COMMAND("sample_job", "i[client id]", ComSampleJob, this, "launch a job for a player in the worker thread (just an example)")
